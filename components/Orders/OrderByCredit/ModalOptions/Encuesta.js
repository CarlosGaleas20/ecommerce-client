...
import { getEncuestaByCard, registerEncuestaByCard } from '../../../../api/infoSell';
...
const Encuesta = ({ order }) => {
    ...
    useEffect(() => {
        (async () => {
            if (auth) {
                const response = await getEncuestaByCard(auth.idUser, order.id, logout);
    ...
    const handleSubmit = async(e) => {
        e.preventDefault();
        setLoading(true);
        if(size(data.titulo) > 0 && size(data.comentario) > 0){
            const response = await registerEncuestaByCard(auth.idUser, order.id, data, logout);
